...
// Statistiche settimanali (utilizza gli eventi già filtrati dal componente padre)
export const calculateWeekStats = (events: EventType[]): WeekStats => {
  const today = new Date()
  const weekStart = new Date(today)
  weekStart.setDate(today.getDate() - 6) // Ultimi 7 giorni (inizia da 7 giorni fa)

  const weekEnd = new Date(today)
  weekEnd.setHours(23, 59, 59, 999) // Fine di oggi

  // Calcola stats dagli eventi già filtrati (non filtra di nuovo)
  let totalMinutes = 0
  events.forEach((event) => {
