...
export const detectAllConflicts = (events: EventType[]): Conflict[] => {
  ...
  // Escludi eventi giornalieri dalla rilevazione conflitti
  const timedEvents = events.filter(event => !event.all_day)

  // Rileva tutti i tipi di conflitti solo per eventi con orari specifici
  conflicts.push(...detectDirectOverlaps(timedEvents))
  conflicts.push(...detectDailyOverload(timedEvents))
  conflicts.push(...detectRecoveryNeeded(timedEvents))
